...
@Query(value = "SELECT DISTINCT d.district_id, d.map_type FROM districts d JOIN district_counties c ON d.district_id = c.district_id AND d.map_type = c.map_type WHERE LOWER(c.counties) LIKE LOWER(CONCAT('%', :county, '%'))", 
       nativeQuery = true)
